...
def build_post_data(hash)
  info        = hash.to_json unless hash.is_a? String
...
class InstagramImage
  ...
  def comment(text)
    post_data = build_post_data({comment_text: text})
  ...
  def follow_author
    ...
    @session.post(follow_user_path(@user_id), post_data)
  end
